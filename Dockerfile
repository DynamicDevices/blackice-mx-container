...
RUN DEBIAN_FRONTEND=noninteractive apt-get -y install build-essential clang bison flex libreadline-dev gawk tcl-dev \
    ...
    qt5-default python3-dev libboost-all-dev cmake libeigen3-dev sudo

# Setup 'build' user with sudo privileges (no password)
...
RUN cd icestorm && make -j$(nproc) && sudo make install
...
RUN cd nextpnr && cmake -DARCH=ice40 -DCMAKE_INSTALL_PREFIX=/usr/local . && make -j$(nproc) && sudo make install
...
RUN DEBIAN_FRONTEND=noninteractive sudo apt-get -y install autoconf libusb-1.0
RUN git clone https://github.com/mystorm-org/dfu-util.git
RUN cd dfu-util && ./autogen.sh && ./configure && make && sudo make install
...
RUN sudo echo "deb https://dl.bintray.com/sbt/debian /" | sudo tee -a /etc/apt/sources.list.d/sbt.list
...
RUN sudo apt-get install -y autoconf automake autotools-dev curl libmpc-dev \
        libmpfr-dev libgmp-dev gawk build-essential bison flex texinfo \
        gperf libtool patchutils bc zlib1g-dev git libexpat1-dev

RUN sudo mkdir /opt/riscv32i && sudo chown build:build /opt/riscv32i
...
RUN cd riscv-gnu-toolchain-rv32i && mkdir build && cd build && ../configure --with-arch=rv32i --prefix=/opt/riscv32i 
RUN cd riscv-gnu-toolchain-rv32i && cd build && make -j$(nproc)
